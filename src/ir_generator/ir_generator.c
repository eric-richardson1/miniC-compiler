...
void generateStmtIR(astNode *node, LLVMModuleRef module, std::unordered_map<string, LLVMValueRef> &ptr_map, std::unordered_map<std::string, LLVMValueRef> &func_map, LLVMBuilderRef &builder, LLVMValueRef func);
LLVMValueRef generate(astNode *node, LLVMModuleRef module, std::unordered_map<string, LLVMValueRef> &ptr_map, std::unordered_map<std::string, LLVMValueRef> &func_map, LLVMBuilderRef builder);
void generateNodeIR(astNode *node, LLVMModuleRef module, std::unordered_map<string, LLVMValueRef> &ptr_map, std::unordered_map<std::string, LLVMValueRef> &func_map, LLVMBuilderRef builder, LLVMValueRef func);
#define prt(x) if(x) { printf("%s\n", x); }



void generateIR(astNode *root, const char *module_name) {
    ...
    std::unordered_map<std::string, LLVMValueRef> func_map;
    ...
    std::pair<std::string, LLVMValueRef> print_entry ("print", extern_print);
    std::pair<std::string, LLVMValueRef> read_entry ("read", extern_read);
    func_map.insert(print_entry);
    func_map.insert(read_entry);

    generateNodeIR(root, module, ptr_map, func_map, builder, func);
...
void generateNodeIR(astNode *node, LLVMModuleRef module, std::unordered_map<string, LLVMValueRef> &ptr_map, std::unordered_map<std::string, LLVMValueRef> &func_map, LLVMBuilderRef builder, LLVMValueRef func) {
    switch (node->type) {
        case ast_prog: {
            generateNodeIR(node->prog.func, module, ptr_map, func_map, builder, func);
            break;
        }
        ...
        case ast_func: {
            ...
            generateNodeIR(node->func.body, module, ptr_map, func_map, builder, func);
            break;
        }
        case ast_stmt: {
            generateStmtIR(node, module, ptr_map, func_map, builder, func);
            break;
        }
    }
	...
	switch (node->stmt.type) {
		...
		case ast_block: {
			...
			for (int i = 0; i < slist->size(); i++) {
				...
				generateNodeIR(slist->at(i), module, ptr_map, func_map, builder, func);
			}
			break;
		}
		case ast_decl: {
		...
		case ast_asgn: {
            LLVMBuildStore(builder, generate(node->stmt.asgn.rhs, module, ptr_map, func_map, builder), ptr_map.at(node->stmt.asgn.lhs->var.name));
			break;

		}
		case ast_if: {
            LLVMValueRef cond = generate(node->stmt.ifn.cond, module, ptr_map, func_map, builder);
            ...
            if (node->stmt.ifn.else_body != NULL) {
                ...
                generateNodeIR(node->stmt.ifn.if_body, module, ptr_map, func_map, builder, func);
                LLVMBuildBr(builder, final);

                LLVMPositionBuilderAtEnd(builder, else_BB);
                generateNodeIR(node->stmt.ifn.else_body, module, ptr_map, func_map, builder, func);
                LLVMBuildBr(builder, final);
		...
		case ast_while: {
            ...
            LLVMValueRef cond = generate(node->stmt.whilen.cond, module, ptr_map, func_map, builder);
            LLVMBuildCondBr(builder, cond, while_body, final);

            LLVMPositionBuilderAtEnd(builder, while_body);
            generateNodeIR(node->stmt.whilen.body, module, ptr_map, func_map, builder, func);
            LLVMBuildBr(builder, check_BB);
		...
		case ast_call: {
            generate(node, module, ptr_map, func_map, builder);
			break;
		}
		case ast_ret: {
            LLVMValueRef ret_val = generate(node->stmt.ret.expr, module, ptr_map, func_map, builder);
...
LLVMValueRef generate(astNode *node, LLVMModuleRef module, std::unordered_map<string, LLVMValueRef> &ptr_map, std::unordered_map<std::string, LLVMValueRef> &func_map, LLVMBuilderRef builder) {
    switch (node->type) {
        case ast_bexpr: {
            LLVMValueRef lhs = generate(node->bexpr.lhs, module, ptr_map, func_map, builder);
            LLVMValueRef rhs = generate(node->bexpr.rhs, module, ptr_map, func_map, builder);
        ...
        case ast_uexpr: {
            LLVMValueRef expr = generate(node->uexpr.expr, module, ptr_map, func_map, builder);
        ...
        case ast_rexpr: {
            LLVMValueRef lhs = generate(node->rexpr.lhs, module, ptr_map, func_map, builder);
            LLVMValueRef rhs = generate(node->rexpr.rhs, module, ptr_map, func_map, builder);
            if (node->rexpr.op == lt) {
        ...
        case ast_stmt: {
                ...
                if (node->stmt.call.param == NULL) {
                    ...
                    param[0] = generate(node->stmt.call.param, module, ptr_map, func_map, builder);
